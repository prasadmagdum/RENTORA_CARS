...
const ManageBookings = () => {
  ...
  const fetchOwnerBookings = async () => {
    try {
      ...
      if (data.success) {
        // filter out bookings with null car to prevent errors
        setBooking(data.data.filter((b) => b.car));
      } else {
        ...
        <table className="w-full border-collapse text-left text-sm text-gray-600">
          ...
          <tbody>
            {bookings.map((booking, index) => {
              ...
              if (!car) return null; // skip if car is null
              ...
              return (
                <tr key={booking._id || index} className="border-t border-borderColor text-gray-500">
                  <td className="p-3 flex items-center gap-3">
                    ...
                    />
                    <p className="font-medium max-md:hidden">{car.brand} {car.model}</p>
                  </td>
